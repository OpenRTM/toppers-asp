...
#  オプションの処理
#
OptionParser.new(nil, __LINE__) do |opt|
  ...
  opt.on("-u applasmobjs",		"additional application asm object files") do |val|
    $applasmobjs += val.split(/\s+/)
  end
  opt.on("-X applcxxobjs",		"additional application cxx object files") do |val|
    $applcxxobjs += val.split(/\s+/)
  end
  ...
  opt.on("-P project",		"project config ") do |val|
    $project_file = val
  end
  opt.parse!(ARGV)
end
...
if $project_file
  path = $project_file
  path += ".rb" unless File.extname(path) == ".rb"
  $csrc_files =[]
  $cxxsrc_files=[]
  $asmsrc_files=[]
  $src_dirs=[]
  ...
  if Config::TEMPMAKEFILE
    $tempmakefile=Config::TEMPMAKEFILE
  end
  
  if Config::LIBS
    Config::LIBS.each_line do |line|
      libelm = line.strip
      next if libelm.empty? || libelm.start_with?('#')
      $libs << libelm
    end
  end
  if Config::CDEFS
    Config::CDEFS.each_line do |line|
      defelm = line.strip
      next if defelm.empty? || defelm.start_with?('#')
      $cdefs << defelm
    end 
  end 
  if Config::LDFLAGS
  Config::LDFLAGS.each_line do |line|
    defelm = line.strip
    next if defelm.empty? || defelm.start_with?('#')
    $ldflags << defelm
  end 
  ...
  if Config::APPL_DIRS
    $appldirs += Config::APPL_DIRS.split(/\s+/)
  end
  if Config::SRC_FILES
    Config::SRC_FILES.each_line do |line|
      fname = line.strip
      next if fname.empty? || fname.start_with?('#')
      case File.extname(fname).downcase
      when ".c" 
        $csrc_files << File.basename(fname,File.extname(fname))+".o"
        $src_dirs << File.dirname(fname)
      when ".cpp"
        $cxxsrc_files << File.basename(fname,File.extname(fname))+".o"
        $src_dirs << File.dirname(fname)
      when ".s"
        $asmsrc_files << File.basename(fname,File.extname(fname))+".o"
        $src_dirs << File.dirname(fname)
      else other << fname
      end
    end
  ...
  if $cxxsrc_files 
    $applcxxobjs += $cxxsrc_files
  end
  if $asmsrc_files
    $applasmobjs += $asmsrc_files
  end
  if $src_dirs
    $src_dirs.uniq!
    $appldirs += $src_dirs
...
$vartable["APPLCXXOBJS"] = $applcxxobjs.join(" ")
$vartable["APPLASMOBJS"] = $applasmobjs.join(" ")
